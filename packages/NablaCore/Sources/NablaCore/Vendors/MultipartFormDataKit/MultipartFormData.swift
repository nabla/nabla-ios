...
public struct MultipartFormData {
        ...
        .from(
            ...
            parameters: [
                ContentType.Parameter(
                    attribute: "boundary",
                    value: uniqueAndValidLengthBoundary
                ),
            ]
        )
    }
    ...
    public func asData() -> ValidationResult<Data, DataTransformError> {
        var data = Data()
        guard let boundaryData = uniqueAndValidLengthBoundary.data(using: .utf8) else {
            return .invalid(because: .boundaryError(debugInfo: uniqueAndValidLengthBoundary))
        }
